...
export default function SearchBar (){
    ...
    useEffect(() => {
        if (isSearchClicked && !isLoading && searchResults) {
    ...
    const clearSearchTermButton = (
        <span
            ...
            className="flex items-center text-white text-lg bg-gray-700 
                border-solid border-2 border-r-0 border-l-0 border-orange-500 
                cursor-pointer"
        >
            <MdOutlineCancel />
        </span>
    ...
    const spinnerDiv = 
            <div className="flex justify-center">
                <ThreeCircles color="#f97316" />
            </div>;

    const noResultsDiv = (
        <div>
            <p className="text-white">There were no matches to your search</p>
        </div>
    );
            ...
            <div className="pt-5 md:flex md:flex-col justify-center items-center">
                <div className="flex justify-center p-5 md:w-3/5">
                    <input
                        ...
                        className="bg-gray-700 border-2 border-r-0 
                                border-orange-500 rounded-l-lg h-10 
                                text-white w-3/5 p-1"
                    />
                    {searchTerm && clearSearchTermButton}
                    <div
                        onClick={handleSearchButton}
                        className="px-3 bg-gray-700 flex items-center 
                            text-white border-2 rounded-r-lg border-l-0 
